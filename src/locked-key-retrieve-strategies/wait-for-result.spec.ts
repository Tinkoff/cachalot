import { DEFAULT_MAXIMUM_TIMEOUT, DEFAULT_REQUEST_TIMEOUT, WaitForResultLockedKeyRetrieveStrategy } from './wait-for-result';
...
describe('WaitForResultLockedKeyRetrieveStrategy', () => {
  ...
  it('get throws if null in cache', async () => {
    ...
    await expect(instance.get({ key: 'test' }))
      .rejects.toThrow('Error while waiting for result in cache');
  ...
  it('get throws if maximum timeout exceeded, while trying to access locked key', async () => {
    ...
    await expect(instance.get({ key: 'test' }))
      .rejects.toThrow('Exceeded maximum timeout of 100');
