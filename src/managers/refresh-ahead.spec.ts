import RefreshAheadManager from './refresh-ahead';
...
describe('RefreshAheadManager', () => {
  ...
  it('constructor throws if Refresh-Ahead factor is less or equals zero', () => {
    expect(() => new RefreshAheadManager({
      storage,
      prefix: 'cache',
      hashKeys: false,
      expiresIn: 10000,
  ...
  it('get refreshes record if storage record will expire soon and not throws if executor throws', async () =>
    new Promise(async (resolve, reject): Promise<any> => {
      ...
      storage.get.mockResolvedValueOnce(returnMock);
      expect(await manager.get('test', () => { throw new Error('Operation timeout'); })).toEqual('234');
      Date.now = realNow;
    }));
